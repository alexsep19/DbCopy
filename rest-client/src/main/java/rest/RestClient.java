...
public class RestClient {
    ...
    static void sendGet(String postPath){
        try(CloseableHttpClient client = HttpClientBuilder.create().build()){
            HttpGet httpGet = new HttpGet(URL + postPath + "?offset=0&length=1");
            httpGet.addHeader("User-Agent", USER_AGENT);
//            List<NameValuePair> urlParameters = new ArrayList<NameValuePair>();
//            urlParameters.add(new BasicNameValuePair("offset", "0"));
//            urlParameters.add(new BasicNameValuePair("length", "1"));
//            httpGet.setEntity(new UrlEncodedFormEntity(urlParameters));
            ...
            users = mapper.readValue( line, new TypeReference<List<User>>(){});
    ...
    static void sendPost(String postPath){
        try(CloseableHttpClient client = HttpClientBuilder.create().build()){
